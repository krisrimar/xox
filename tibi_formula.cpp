...
string option;
bool trueFalse = true; // for while functions in code

inline int amountOfWater();
inline int amountOfDegrees();
inline int errorNotACommand();
inline void lineBreak();


// line break after each new option

void lineBreak () {
    cout << "\n------------------\n\n";
}

// the menu for choosing different formulas

int menu(void) {

    string menuNumber;

    cout << "(!) Для виклику меню, замість значень в формулах введіть букву M та натисніть Enter. (!) \n(!) Для виходу з програми введіть 0 та натисніть Enter (!) \n\n";

    cout << "Виберіть формулу, введіть її номер та натисніть Enter: \n \n";
    cout << "1. Визначення к-сті води; \n";
    cout << "2. Визначення загальної к-сті градусів; \n \n";
    cout << "Введіть число: ";
    cin >> menuNumber;
    ...
    while(trueFalse) {
        ...
        cout << "Нажаль, такого пункту не існує. Повторіть спробу: ";
        cin >> menuNumber;

    }
}

// the formula for calculating the amount of water needed to lower the degrees of spirit

int amountOfWater(void) {

    double a, b, c, sum;
    string answer;

    cout << "(!) Використовуйте крапку для не цiлих чисел. Наприклад: 92.5 (!) \n \n";
    cout << "Наявна мiцнiсть спирту, у вiдсотках (%): ";
    cin >> a;
    cout << "\nНаявний обсяг спирту, у мілілітрах (мл, ml): ";
    cin >> b;
    cout << "\nБажана міцність спирту, у відсотках (%): ";
    cin >> c;
    b = b / 100;
    ...
    cout << "Додайте до " << b / 10 << " л (" << a << "°) спирту --> " << sum << " мл <-- води, щоб отримати " << c << "° спирт \n\n";

    cout << "Провести ще один розрахунок? Y - так N - нi (M - повернутись до меню): ";
    cin >> answer;
...
// function for calculating the average amount of spirit if mixing two or more batches

int amountOfDegrees(void) {
...
int errorNotACommand(void) {

    cout << "Введене значення не є командою. Введіть правильне значення (M - повернутись до меню) та натисніть Enter: ";
    cin >> option;
    if (option == 'M' || option == 'm') {
        lineBreak();
        menu();
...
// main function that starts to run menu on app start up
